  ...
  for (const item of items) {
    ...
    if (item.variant) {
      ...
      if (variant.product.toString() !== product._id.toString()) {
        throw new Error(
          `Variant ${item.variant} does not belong to product ${product._id}`,
        );
      }
    }
    ...
    if (stockToCheck < item.quantity) {
      throw new Error(
        `Not enough stock for ${product.name}${variant ? " (Variant)" : ""}`,
      );
    }

    if (basePrice <= 0 && !variant) {
      throw new Error(
        `Product ${product.name} must have a variant selected for pricing`,
      );
    }
  ...
  if (newStatus === "CONFIRMED" && oldStatus === "PENDING") {
    for (const item of order.items) {
      if (item.variant) {
        ...
        if (
          !variantFound ||
          variantFound.product.toString() !== item.product.toString()
        ) {
          throw new Error("Invalid variant for product");
        }
        if (variantFound.stock < item.quantity) {
          throw new Error(
            `Insufficient stock for variant ${variantFound.sku || item.variant}`,
          );
        }
        ...
        if (productFound.stock < item.quantity) {
          throw new Error(
            `Insufficient stock for product ${productFound.name}`,
          );
        }
