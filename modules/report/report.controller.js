...
export const getMyReports = asyncHandler(async (req, res) => {
  const { page = 1, limit = 50 } = req.query;
  const { data, total } = await reportService.getUserReports(
    req.user._id,
    req.query,
  );
  res.json({
...
export const getShopReports = asyncHandler(async (req, res) => {
  ...
  const { page = 1, limit = 50 } = req.query;
  const { data, total } = await reportService.getShopRelatedReports(
    shopId,
    req.query,
  );
  res.json({
...
export const getPendingCount = asyncHandler(async (req, res) => {
  const { shopId } = req.params;
  const { total } = await reportService.getShopRelatedReports(shopId, {
    status: "pending",
    limit: 1,
  });
